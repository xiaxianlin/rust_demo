...
fn add_edge<V: Ord + Copy, E: Ord + Copy + Add>(graph: &mut Graph<V, E>, v1: V, v2: V, c: E) {
    graph.entry(v1).or_insert_with(BTreeMap::new).insert(v2, c);
    graph.entry(v2).or_insert_with(BTreeMap::new).insert(v1, c);
}

fn prim_with_start<V: Ord + Copy, E: Ord + Copy + Add>(
    graph: &Graph<V, E>,
    start: V,
) -> Graph<V, E> {
...
pub fn prim<V: Ord + Copy + Debug, E: Ord + Copy + Add + Debug>(
    graph: &Graph<V, E>,
) -> Graph<V, E> {
